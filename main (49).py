...
def _sanitize_title_for_api(title: str) -> str:
    """
    Limpieza profunda del título justo antes de enviarlo a APIs externas
    (TMDB, Google Knowledge Graph, Gemini, TVMaze).

    Elimina símbolos decorativos, subtítulos separados por guión, versiones
    alternativas separadas por barra, y caracteres especiales que reducen
    la precisión de búsqueda.

    Ejemplos:
      "### EL EVANGELIO SEGÚN MATEO - LA VIDA DE JESÚS"
          → "EL EVANGELIO SEGÚN MATEO"

      "** Rambo: First Blood ** (HD) [Latino]"
          → "Rambo: First Blood"
      ...
      "Spider-Man: No Way Home - Edición Extendida"
          → "Spider-Man: No Way Home"

      "Fast - X"
          → "Fast - X"   (se respeta porque la parte principal es muy corta)
    """
    if not title:
        return title
    ...
    # 1. Eliminar bloques repetidos de símbolos decorativos al INICIO
    #    Cubre: ###, ---, ***, ===, ~~~, +++, etc.
    t = re.sub(r'^[\s#\-=*_~+|<>•·]{2,}', '', t).strip()

    # 2. Eliminar esos mismos bloques al FINAL
    t = re.sub(r'[\s#\-=*_~+|<>•·]{2,}$', '', t).strip()

    # 3. Cortar versión alternativa después de " / " ANTES de eliminar la barra.
    #    "La Era del Hielo 3 / Ice Age 3" → "La Era del Hielo 3"
    #    Solo si la parte principal tiene al menos 5 caracteres.
    _slash_split = re.split(r'\s+/\s+', t, maxsplit=1)
    if len(_slash_split) == 2 and len(_slash_split[0].strip()) >= 5:
        t = _slash_split[0].strip()

    # 4. Cortar subtítulo después de " - " cuando la parte principal es
    #    suficientemente larga (≥ 15 caracteres O ≥ 3 palabras significativas).
    #    Esto convierte:
    #      "EL EVANGELIO SEGÚN MATEO - LA VIDA DE JESÚS" → "EL EVANGELIO SEGÚN MATEO"
    #      "Spider-Man: No Way Home - Edición Extendida"  → "Spider-Man: No Way Home"
    #    Pero NO corta títulos cortos como "Fast - X" donde ambas partes son del título.
    _dash_split = re.split(r'\s+[-–—]\s+', t, maxsplit=1)
    if len(_dash_split) == 2:
        _main = _dash_split[0].strip()
        _main_words = [w for w in _main.split() if len(w) > 2]
        if len(_main) >= 15 or len(_main_words) >= 3:
            t = _main

    # 5. Eliminar caracteres especiales sueltos que no forman parte del título.
    #    Se preservan: letras (incluyendo acentos/ñ), dígitos, espacio, guión simple,
    #    dos puntos, punto, paréntesis, interrogación ¿?, exclamación ¡!
    t = re.sub(r'[#@\$\*"\\&%^=~`\+]+', ' ', t)

    # 6. Eliminar barras residuales sueltas (que no se pudieron tratar en paso 3)
    t = re.sub(r'\s*/\s*', ' ', t)

    # 7. Eliminar extensiones de archivo que a veces quedan en el título
    t = re.sub(
        r'\.(mp4|mkv|avi|mov|wmv|flv|webm|m4v|3gp|mpeg|mpg)\b',
        '', t, flags=re.IGNORECASE,
    )

    # 8. Limpiar espacios múltiples residuales
    t = re.sub(r'\s+', ' ', t).strip()

    # 9. Devolver el resultado; si quedó vacío usar el original
    return t or title
...
def _build_tmdb_query_from_title(title: str):
    ...
    if len(q.strip()) < 3:
        ...
        # ✅ Aplicar sanitización profunda también al fallback
        q = _sanitize_title_for_api(q)
        return (q or raw), (detected_year or year_fallback)

    q = re.sub(r"\s+", " ", q).strip()

    # ✅ MEJORA: Aplicar sanitización profunda al query final antes de enviarlo
    # a TMDB, Google KG, Gemini y TVMaze. Elimina símbolos, subtítulos y caracteres
    # especiales que reducen la precisión de búsqueda en las APIs.
    q = _sanitize_title_for_api(q)
