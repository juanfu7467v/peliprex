...
# ---------------------------------------------------------------------------
# STREAMING (FIX DEFINITIVO — ESTABLE)
# ---------------------------------------------------------------------------
# STREAM_CHUNK_SIZE: tamaño de cada chunk enviado al cliente (1 MB por defecto).
# STREAM_REQUEST_SIZE: tamaño de cada petición interna de Telethon a Telegram.
# Usar 1 MB en ambos reduce los round-trips a Telegram y estabiliza el buffer del player.
...
STREAM_REQUEST_SIZE = max(
    128 * 1024,
    min(2 * 1024 * 1024, int(os.getenv("STREAM_REQUEST_SIZE", str(1024 * 1024))))
)
...
# ---------------------------------------------------------------------------
# ✅ FIX 2: Validador de tipo de entidad — evita PeerUser / Invalid channel object
# ---------------------------------------------------------------------------
def _is_searchable_entity(entity) -> bool:
    """
    Retorna True si la entidad es un Canal o Grupo válido para búsqueda.
    Retorna False para Usuarios (PeerUser), lo que evita errores de tipo.
    Soporta: Channel, Chat, InputPeerChannel, InputPeerChat.
    También acepta Users (búsqueda directa en chat privado, sin search=).
    """
    if entity is None:
        return False
    try:
        from telethon.tl.types import (
            Channel, Chat, InputPeerChannel, InputPeerChat,
            InputPeerUser, User,
        )
        # Siempre se pueden buscar — usuarios en chats privados también tienen videos
        return True
    except ImportError:
...
def _entity_is_user(entity) -> bool:
    """Detecta si la entidad es un usuario (chat privado)."""
    try:
...
def _entity_is_channel(entity) -> bool:
    """Detecta si la entidad es un canal o megagrupo."""
    try:
        from telethon.tl.types import Channel
        return isinstance(entity, Channel)
    except ImportError:
        entity_type = type(entity).__name__
        return "Channel" in entity_type


def _entity_is_chat(entity) -> bool:
    """Detecta si la entidad es un grupo normal (Chat)."""
    try:
        from telethon.tl.types import Chat
        return isinstance(entity, Chat)
    except ImportError:
        entity_type = type(entity).__name__
        return "Chat" in entity_type and "Channel" not in entity_type
...
def _db_init_search_cache() -> None:
    """Crea las tablas de caché de búsquedas e índice de películas en SQLite."""
    try:
        ...
        """)
        # ✅ FIX 6: Índice local de películas para búsqueda ultrarrápida (~0.02s)
        con.execute("""
            CREATE TABLE IF NOT EXISTS movie_index (
                ...
                size_mb     TEXT    DEFAULT 'n/a',
                indexed_at  REAL    NOT NULL,
                UNIQUE(channel_idx, msg_id)
            )
        """)
        con.execute("CREATE INDEX IF NOT EXISTS idx_movie_title ON movie_index(title_norm)")
        con.execute("CREATE INDEX IF NOT EXISTS idx_movie_channel ON movie_index(channel_idx)")
        con.commit()
        con.close()
        print(f"✅ BD caché de búsquedas e índice de películas inicializados: {DB_PATH}")
...
# ---------------------------------------------------------------------------
# ✅ FIX 6: ÍNDICE LOCAL DE PELÍCULAS — búsqueda en ~0.02s
# ---------------------------------------------------------------------------
def _db_index_upsert(channel_idx: int, msg_id: int, title_raw: str,
                      caption: str, stream_url: str, size_mb: str) -> None:
    """Inserta o actualiza una película en el índice local SQLite."""
    try:
        title_norm = normalize_title(title_raw)
        con = sqlite3.connect(DB_PATH, check_same_thread=False)
        con.execute("""
            INSERT OR REPLACE INTO movie_index
                (channel_idx, msg_id, title_raw, title_norm, caption, stream_url, size_mb, indexed_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (channel_idx, msg_id, title_raw, title_norm,
              caption[:500], stream_url, size_mb, time.time()))
        con.commit()
        con.close()
    except Exception:
        pass
...
def _db_index_search(query: str, limit: int = 50) -> list:
    """
    Busca en el índice local de películas.
    Usa LIKE para coincidencia parcial + fuzzy matching en Python.
    Retorna lista de dicts con los campos necesarios para el endpoint /search.
    Tiempo esperado: ~0.02s para miles de registros.
    """
    if not query:
        return []
    q_norm = normalize_title(query)
    # Dividir la query en palabras para búsqueda AND parcial
    words = [w for w in q_norm.split() if len(w) >= 2]
    if not words:
        return []
    try:
        con = sqlite3.connect(DB_PATH, check_same_thread=False)
        # Construir LIKE para cada palabra
        conditions = " AND ".join([f"title_norm LIKE ?" for _ in words])
        params = [f"%{w}%" for w in words]
        params.append(limit * 5)  # Traer más para filtrar con fuzzy
        rows = con.execute(
            f"SELECT channel_idx, msg_id, title_raw, caption, stream_url, size_mb "
            f"FROM movie_index WHERE {conditions} LIMIT ?",
            params,
        ).fetchall()
        con.close()
        ...
        results = []
        for row in rows:
            ch_idx, msg_id, title_raw, caption, stream_url, size_mb = row
            if _fuzzy_title_match(query, title_raw):
                results.append({
                    "id":          msg_id,
                    "title":       _extract_title_from_caption(title_raw),
        ...
        print(f"⚠️  Error en búsqueda de índice local: {_e}")
        return []


def _db_index_count() -> int:
    """Retorna el número de películas indexadas."""
    try:
        con = sqlite3.connect(DB_PATH, check_same_thread=False)
        count = con.execute("SELECT COUNT(*) FROM movie_index").fetchone()[0]
        con.close()
        return count
    except Exception:
        return 0
...
async def _db_index_populate_channel(ch_index: int, entity, tg_client,
                                      batch_size: int = 200) -> int:
    """
    Escanea un canal/grupo/usuario y guarda TODOS los videos en el índice local.
    Retorna el número de videos indexados.
    Solo indexa mensajes más nuevos que los ya indexados (incremental).
    """
    if entity is None:
        return 0
    count = 0
    try:
        # Obtener el último msg_id ya indexado para este canal (actualización incremental)
        try:
            con = sqlite3.connect(DB_PATH, check_same_thread=False)
            row = con.execute(
                "SELECT MAX(msg_id) FROM movie_index WHERE channel_idx = ?",
                (ch_index,)
            ).fetchone()
            con.close()
            last_indexed_id = row[0] if row and row[0] else 0
        except Exception:
            last_indexed_id = 0

        async for message in tg_client.iter_messages(entity, limit=batch_size):
            # Solo mensajes con video o documento multimedia
            if not (message.media and (message.video or message.document)):
                continue
            # Saltar mensajes ya indexados
            if message.id <= last_indexed_id:
                break
            caption = message.text or ""
            title   = _extract_title_from_caption(caption)
            public_url_base = PUBLIC_URL or ""
            stream_url = (
                f"{public_url_base}/stream/{message.id}?ch={ch_index}"
                if public_url_base else f"/stream/{message.id}?ch={ch_index}"
            )
            size_mb = (
            ...
            await asyncio.to_thread(
                _db_index_upsert,
                ch_index, message.id, title, caption, stream_url, size_mb
            )
            count += 1

    except FloodWaitError as _fw:
        wait_s = getattr(_fw, "seconds", 15)
        print(f"   ⏳ FloodWait indexando canal {ch_index}: esperando {wait_s}s...")
        await asyncio.sleep(wait_s + 2)
    except Exception as _e:
        print(f"   ⚠️  Error indexando canal {ch_index}: {_e}")
    return count


async def _db_index_populate_all() -> None:
    """
    Tarea de fondo: indexa todos los canales disponibles en el índice local.
    Se ejecuta al iniciar el servidor. Las búsquedas posteriores usarán el índice.
    """
    try:
        # Esperar a que los canales estén listos
        waited = 0.0
        while not getattr(app.state, "channels_ready", False) and waited < 30.0:
            await asyncio.sleep(1.0)
            waited += 1.0

        entities = getattr(app.state, "entities", [])
        total_indexed = 0
        sem = asyncio.Semaphore(3)  # Máximo 3 canales en paralelo para no causar flood

        async def _index_one(ch_idx: int, entity):
            nonlocal total_indexed
            if entity is None:
                return
            async with sem:
                try:
                    _client = _get_next_client()
                    # Re-resolver entidad para evitar "Invalid channel object"
                    try:
                        _ref = getattr(entity, "id", None) or entity
                        entity_resolved = await _client.get_entity(_ref)
                    except Exception:
                        entity_resolved = entity
                    n = await _db_index_populate_channel(ch_idx, entity_resolved, _client)
                    if n > 0:
                        total_indexed += n
                        print(f"   📥 Canal [{ch_idx}] indexado: {n} videos nuevos")
                except Exception as _e:
                    print(f"   ⚠️  Error en indexación de canal {ch_idx}: {_e}")

        tasks = [_index_one(i, e) for i, e in enumerate(entities) if e is not None]
        await asyncio.gather(*tasks, return_exceptions=True)
        print(f"🗂️  Indexación completa: {total_indexed} videos nuevos — "
              f"Total en índice: {_db_index_count()}")
    except Exception as _e:
        print(f"⚠️  Error en indexación global: {_e}")
...
async def lifespan(app: FastAPI):
    ...
    async def _load_backup_channels():
        # ✅ FIX 3: Usar todos los clientes activos para resolver canales en paralelo
        sem = asyncio.Semaphore(5)

        async def _load_one(ch, assigned_client):
            async with sem:
                try:
                    entity = await assigned_client.get_entity(ch)
                    return entity
                except FloodWaitError as _fw:
                    wait_s = getattr(_fw, "seconds", 10)
                    await asyncio.sleep(wait_s + 2)
                    try:
                        return await assigned_client.get_entity(ch)
                    except Exception as _ex2:
                        print(f"⚠️  No se pudo cargar canal {ch} tras espera: {_ex2}")
                        return None
                except Exception as ex:
                    print(f"⚠️  No se pudo cargar canal {ch}: {ex}")
                    return None

        # Distribuir canales entre todos los clientes conectados
        active_clients = _clients_connected if _clients_connected else [client]
        tasks = []
        for pos, ch in enumerate(BACKUP_CHANNELS):
            assigned = active_clients[pos % len(active_clients)]
            tasks.append(_load_one(ch, assigned))

        backup_entities = await asyncio.gather(*tasks, return_exceptions=True)
        backup_entities = [e if isinstance(e, object) and not isinstance(e, Exception) else None
                           for e in backup_entities]
        ...
        # ✅ FIX 6: Iniciar indexación de películas en segundo plano
        asyncio.create_task(_db_index_populate_all())
...
async def search(
    ...
    # ✅ FIX 6: Consultar el índice local de películas PRIMERO (ultrarrápido ~0.02s)
    # Si hay resultados en el índice Y no hay filtros de año/género/idioma que requieran
    # enriquecimiento, devolver directamente desde el índice sin tocar Telegram.
    if effective_text and len(effective_text.strip()) >= 2:
        _index_count = await asyncio.to_thread(_db_index_count)
        if _index_count > 0:
            print(f"⚡ Índice local activo ({_index_count} películas) — buscando en índice primero...")
            _index_hits = await asyncio.to_thread(_db_index_search, effective_text, 50)
            if _index_hits:
                print(f"⚡ Índice local: {len(_index_hits)} resultados encontrados")
                # Continuar con enriquecimiento y filtros, pero usar los resultados del índice
                # como base (más rápido que escanear Telegram en vivo)
                try:
                    _idx_enriched = await asyncio.wait_for(
                        enrich_results_with_tmdb(_index_hits, max_new=MAX_ENRICH_NEW, use_gemini=True),
                        timeout=4.0,
                    )
                except asyncio.TimeoutError:
                    _idx_enriched = _format_results_without_apis(_index_hits)

                if any([effective_year, effective_genre, language, effective_desde, effective_hasta]):
                    _idx_enriched = _apply_advanced_filters(
                        _idx_enriched, effective_year, effective_genre,
                        language, effective_desde, effective_hasta
                    )

                if _idx_enriched:
                    _idx_schema = _to_peliculas_json_schema(_idx_enriched)
                    # Guardar en caché SQLite para futuras búsquedas idénticas
                    await asyncio.to_thread(_db_search_set, _search_cache_key, _idx_schema)
                    print(f"⚡ Respuesta desde índice local en ~0.02s: {len(_idx_schema)} resultado(s)")
                    return _idx_schema
    ...
    try:
        ...
        async def search_in_channel(ch_index: int, entity, assigned_client) -> list:
            if entity is None:
                return []
            # ✅ FIX 2: Validar tipo de entidad antes de cualquier operación
            if not _is_searchable_entity(entity):
                print(f"   ⚠️  Canal [{ch_index}] no es una entidad válida — omitiendo")
                return []
            ...
            try:
                ...
                # ✅ FIX 2+3: Forzar resolución del canal para evitar "Invalid channel object"
                try:
                    _channel_ref = getattr(entity, "id", None) or entity
                    entity = await _active_client.get_entity(_channel_ref)
                except FloodWaitError as _fw:
                ...
                if effective_text:
                    # ✅ FIX 1: BÚSQUEDA DIRECTA — escanear mensajes con video manualmente.
                    # NO usar search= de Telegram (impreciso, no revisa todo el historial).
                    # Se recorren mensajes con video y se compara caption/texto manualmente
                    # usando normalización (sin acentos, minúsculas) + fuzzy matching.
                    # ✅ FIX 4: Normalizar el texto de búsqueda
                    q_norm = normalize_title(effective_text)

                    # ── Paso A: Consultar primero el índice local SQLite (instantáneo) ──
                    try:
                        index_results = await asyncio.to_thread(_db_index_search, effective_text, 50)
                        # Filtrar solo los del canal actual
                        for item in index_results:
                            # El stream_url contiene ?ch=X — verificar que sea este canal
                            su = item.get("stream_url", "")
                            if f"?ch={ch_index}" in su or f"&ch={ch_index}" in su:
                                results.append(item)
                    except Exception:
                        pass  # El índice es best-effort

                    existing_ids = {r.get("id") for r in results}

                    # ── Paso B: Escanear mensajes directamente en Telegram ──
                    # (captura mensajes nuevos no indexados aún o si el índice está vacío)
                    try:
                        # Determinar cuántos mensajes escanear según si es canal o usuario
                        is_user = _entity_is_user(entity)
                        scan_limit = 300 if is_user else None  # None = todo el historial

                        async for message in _active_client.iter_messages(
                            entity,
                            limit=scan_limit,
                        ):
                            # ✅ FIX 1: Solo mensajes con VIDEO (documento o video nativo)
                            if not (message.media and (message.video or message.document)):
                                continue

                            caption = message.text or ""
                            # ✅ FIX 4: Normalizar el texto del mensaje para comparar
                            caption_norm = normalize_title(caption)
                            title_raw = _extract_title_from_caption(caption)
                            title_norm = normalize_title(title_raw)

                            # Comparación manual: substring normalizado o fuzzy match
                            matched = (
                                q_norm in title_norm
                                or q_norm in caption_norm
                                or title_norm in q_norm
                                or _fuzzy_title_match(effective_text, title_raw)
                                or _fuzzy_title_match(effective_text, caption)
                            )
                            if not matched:
                                continue
                            ...
                            # ✅ También actualizar el índice con mensajes nuevos encontrados
                            asyncio.create_task(asyncio.to_thread(
                                _db_index_upsert,
                                ch_index, message.id, title_raw, caption, direct_link, size_mb
                            ))
                    ...
                    # ── Paso C: Fuzzy scan sobre caché reciente del canal (complementario) ──
                    try:
                ...
                entity_name = getattr(entity, 'title', None) or getattr(entity, 'username', None) or str(ch_index)
                print(f"   📺 Canal [{ch_index}] ({entity_name}): {len(results)} resultado(s)")
            except FloodWaitError as _fw:
                wait_s = getattr(_fw, "seconds", 30)
        ...
        # ✅ FIX 5: BÚSQUEDA PARALELA REAL — dividir canales en grupos, uno por cuenta.
        # ADEMÁS, dentro de cada grupo los canales se buscan en PARALELO con gather().
        # Con 3 cuentas y 35 canales → ~11 canales por cuenta en paralelo simultáneo.
        # Tiempo esperado: 3-8s (vs >60s anterior).
        async def search_group(group_channels: list, account_client) -> list:
            """Busca en un grupo de canales usando una cuenta específica, en paralelo."""
            # ✅ FIX 5: Usar gather() para paralelismo real dentro del grupo
            channel_tasks = [
                search_in_channel(ch_idx, ch_entity, account_client)
                for ch_idx, ch_entity in group_channels
            ]
            results_per_channel = await asyncio.gather(*channel_tasks, return_exceptions=True)
            group_results = []
            for res in results_per_channel:
...
async def catalog():
    try:
        ...
        if isinstance(cached_pool, list) and len(cached_pool) > 0 and (now - cached_ts) < CATALOG_POOL_TTL:
            ...
            # ✅ OPT: Límite reducido a 15 por canal para menor uso de CPU/RAM
            ...
            async def fetch_from_channel(ch_index: int, entity) -> list:
                ...
                try:
                    ...
                    async with fetch_sem:
                        # ✅ ANTI-FLOOD: Pausa aumentada para evitar Flood Wait en catálogo
                        ...
                        try:
                            ...
                            wait_s = getattr(_fw, "seconds", 30)
                            print(f"⏳ FloodWait en catálogo ch={ch_index}: esperando {wait_s}s automáticamente...")
                            await asyncio.sleep(wait_s + 2)
                            # Retornar lo recopilado antes del flood
                except Exception as e:
            ...
            async def chunk_generator_full(offset: int, limit: int):
                try:
                    ...
                    async for chunk in active_client.iter_download(
                        ...
                        buf.extend(chunk)
                        # Enviar en bloques de STREAM_CHUNK_SIZE para un buffer estable
                        while len(buf) >= STREAM_CHUNK_SIZE:
                            yield bytes(buf[:STREAM_CHUNK_SIZE])
