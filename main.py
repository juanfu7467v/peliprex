    ...
    try:
        ...
        async def search_in_channel(ch_index: int, entity, assigned_client) -> list:
            ...
            try:
                ...
                if effective_text:
                    try:
                        # ✅ Sin límite de mensajes — se busca en todo el historial disponible
                    ...
                    # ✅ FUZZY SEARCH: además de la búsqueda nativa de Telegram,
                    # escanear la caché reciente del canal con distancia de Levenshtein.
                    # Esto permite encontrar "La Era del Hielo" cuando el usuario escribe "la era de yelo".
                    try:
        ...
        # ✅ BÚSQUEDA PARALELA: dividir canales en grupos, uno por cuenta.
        # Cada cuenta busca en su grupo de canales de forma simultánea.
        # Con 3 cuentas y 35 canales → ~11 canales por cuenta en paralelo.
        # Esto reduce el tiempo de búsqueda de >60s a ~10s.
        async def search_group(group_channels: list, account_client) -> list:
            """Busca en un grupo de canales usando una cuenta específica."""
            group_results = []
            for ch_idx, ch_entity in group_channels:
                channel_results = await search_in_channel(ch_idx, ch_entity, account_client)
                group_results.extend(channel_results)
