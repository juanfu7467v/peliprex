...
# ✅ NUEVO: Semáforo global de clientes Telegram (máx 15 simultáneas)
CLIENT_SEM_LIMIT = max(5, min(15, int(os.getenv("CLIENT_SEM_LIMIT", "15"))))
...
# ✅ Intentos de descarga de thumb antes de rendirse
THUMB_DOWNLOAD_RETRIES = int(os.getenv("THUMB_DOWNLOAD_RETRIES", "2"))
# Rango de mensajes adyacentes donde buscar póster
THUMB_NEARBY_RANGE = max(2, min(8, int(os.getenv("THUMB_NEARBY_RANGE", "5"))))
...
def _crop_cover_to_poster(image_data: bytes) -> bytes:
    ...
    try:
        ...
        if img.mode not in ("RGB", "RGBA"):
            ...
            bg = _PIL_Image.new("RGB", img.size, (0, 0, 0))
...
# ---------------------------------------------------------------------------
# ✅ _thumb_url_for_message — siempre genera URL si el ID es válido
# ---------------------------------------------------------------------------
...
def _is_placeholder_image(url) -> bool:
    u = (url or "").strip()
    return (not u) or (u == PLACEHOLDER_IMAGE_BASE) or u == "n/a"
...
# ---------------------------------------------------------------------------
# ✅ Cache IA helpers con persistencia
# ---------------------------------------------------------------------------
def _ai_cache_entry_ttl_s(status: str) -> int:
    st = (status or "").lower()
    if st == "ok":    return AI_CACHE_TTL_OK_S
    if st == "none":  return AI_CACHE_TTL_NONE_S
    if st == "429":   return AI_CACHE_TTL_429_S
    if st == "err":   return AI_CACHE_TTL_ERR_S
...
# ---------------------------------------------------------------------------
# ✅ HELPER MEJORADO: Obtener el mejor thumb nativo de Telegram
# ---------------------------------------------------------------------------
def _get_best_native_thumb(thumbs_list):
    """
    Dado una lista de thumbs de Telegram, devuelve el mejor objeto thumb
...
# ---------------------------------------------------------------------------
# ✅ MEJORA CRÍTICA: Limpiar descripción
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ✅ MEJORA CRÍTICA: Buscar póster en mensajes cercanos al video
# ---------------------------------------------------------------------------
async def _find_poster_in_nearby_messages(
    ...
    if search_range is None:
        search_range = THUMB_NEARBY_RANGE
    try:
            ...
            if hasattr(msg, "photo") and msg.photo:
                try:
                    ...
                    if photo_data and len(photo_data) > 500:
                        print(f"   🖼️  Póster encontrado en msg cercano #{msg.id} (distancia={abs(msg.id - message_id)})")
                        return photo_data
                except Exception:
                    continue
...
# ---------------------------------------------------------------------------
# ✅ NUEVO: Descargar thumb nativo de Telegram de forma robusta con reintentos
# ---------------------------------------------------------------------------
async def _download_telegram_thumb_robust(
    message,
    entity,
    tg_client,
    message_id: int,
    ...
    """
    Intenta descargar la miniatura nativa de un mensaje de Telegram
    recorriendo todas las fuentes posibles en orden de prioridad:

    0. Poster en mensajes adyacentes (foto enviada junto al video)
    1. message.photo (mensaje que es una foto)
    2. message.video.thumbs
    3. message.document.thumbs
    4. message.media.thumbs

    Devuelve bytes de la imagen o None si no se encuentra ninguna.
    """
    if message is None:
        return None

    # PRIORIDAD 0: Buscar póster en mensajes cercanos
    try:
        poster = await _find_poster_in_nearby_messages(entity, message_id, tg_client)
        if poster and len(poster) > 500:
            return poster
    except Exception as e:
        print(f"   ⚠️  Error en búsqueda de póster adyacente (msg {message_id}): {e}")

    # PRIORIDAD 1: message.photo
    try:
        if hasattr(message, "photo") and message.photo:
            for attempt in range(THUMB_DOWNLOAD_RETRIES):
                try:
                    data = await asyncio.wait_for(
                        tg_client.download_media(message.photo, bytes),
                        timeout=3.0,
                    )
                    if data and len(data) > 200:
                        print(f"   📷 Thumb P1 (photo) msg {message_id}")
                        return data
                except asyncio.TimeoutError:
                    if attempt < THUMB_DOWNLOAD_RETRIES - 1:
                        await asyncio.sleep(0.3)
                except Exception:
                    break
    except Exception as e:
        print(f"   ⚠️  Error P1 thumb msg {message_id}: {e}")

    # PRIORIDAD 2: message.video.thumbs (Telethon devuelve Document como video)
    try:
                ...
                for attempt in range(THUMB_DOWNLOAD_RETRIES):
                    try:
                        raw = await asyncio.wait_for(
                            tg_client.download_media(video_obj, bytes, thumb=best),
                            timeout=3.0,
                        )
                        if raw and len(raw) > 200:
                            print(f"   🎬 Thumb P2 (video.thumbs) msg {message_id}")
                            return raw
                    except asyncio.TimeoutError:
                        if attempt < THUMB_DOWNLOAD_RETRIES - 1:
                            await asyncio.sleep(0.3)
        ...
        print(f"   ⚠️  Error P2 thumb msg {message_id}: {e}")

    # PRIORIDAD 3: message.document.thumbs
    try:
        if message.document and getattr(message.document, "thumbs", None):
            ...
            if best:
                for attempt in range(THUMB_DOWNLOAD_RETRIES):
                    try:
                        raw = await asyncio.wait_for(
                            tg_client.download_media(message.document, bytes, thumb=best),
                            timeout=3.0,
                        )
                        if raw and len(raw) > 200:
                            print(f"   📎 Thumb P3 (document.thumbs) msg {message_id}")
                            return raw
                    except asyncio.TimeoutError:
                        if attempt < THUMB_DOWNLOAD_RETRIES - 1:
                            await asyncio.sleep(0.3)
        ...
        print(f"   ⚠️  Error P3 thumb msg {message_id}: {e}")

    # PRIORIDAD 4: message.media.thumbs
    try:
        ...
        if media_thumbs:
            ...
            if best:
                for attempt in range(THUMB_DOWNLOAD_RETRIES):
                    try:
                        raw = await asyncio.wait_for(
                            tg_client.download_media(media_obj, bytes, thumb=best),
                            timeout=3.0,
                        )
                        if raw and len(raw) > 200:
                            print(f"   🖼️  Thumb P4 (media.thumbs) msg {message_id}")
                            return raw
                    except asyncio.TimeoutError:
                        if attempt < THUMB_DOWNLOAD_RETRIES - 1:
                            await asyncio.sleep(0.3)
        ...
        print(f"   ⚠️  Error P4 thumb msg {message_id}: {e}")

    # Sin miniatura disponible
    print(f"   ℹ️  Sin miniatura nativa para msg {message_id}")
    return None
...
async def lifespan(app: FastAPI):
    ...
    # ✅ NUEVO: Semáforo global de clientes Telegram para control de concurrencia
    app.state.client_semaphore = asyncio.Semaphore(CLIENT_SEM_LIMIT)
    ...
    print(f"⚙️  IA semáforo: max={AI_SEM_LIMIT} | Client semáforo: max={CLIENT_SEM_LIMIT}")
...
# ---------------------------------------------------------------------------
# ✅ MULTI-CUENTA: Inicialización de clientes Telegram
# ---------------------------------------------------------------------------
...
async def get_active_client() -> TelegramClient | None:
    ...
    Usa el semáforo client_semaphore para control de concurrencia.
    """
...
def _get_next_client() -> TelegramClient:
    """Rotación síncrona Round Robin entre clientes disponibles."""
...
async def health_check():
    ...
    return JSONResponse({
        ...
        "client_sem_limit":    CLIENT_SEM_LIMIT,
    })
...
_NOISE_PATTERNS_ADVANCED = [
    ...
    r"\bfull\s*hd\b", r"\bhd\b", r"\bhdts\b", r"\bhdcam\b",
    r"\bhdrip\b", r"\bwebdl\b", r"\bweb[-\s]?dl\b", r"\bweb[-\s]?rip\b",
    ...
    r"\bsubtitulado\b", r"\bsubtiulado\b", r"\bcastellano\b",
    r"\blatino\b", r"\bespa[ñn]ol\b", r"\bingles\b", r"\benglish\b",
    r"\bdual\b", r"\bdoblaje\b",
...
# ---------------------------------------------------------------------------
# EXTRACCIÓN DE TÍTULO (compatibilidad)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# HELPERS: limpieza de títulos (compatibilidad)
# ---------------------------------------------------------------------------
_NOISE_PATTERNS = [
...
async def _fetch_recent_media_from_channel(ch_index: int, entity, limit: int) -> list:
    ...
    try:
        ...
        # ✅ Usar semáforo de cliente para evitar FloodWait
        client_sem = getattr(app.state, "client_semaphore", asyncio.Semaphore(CLIENT_SEM_LIMIT))
        ...
        print(f"⚠️  Error en _fetch_recent_media_from_channel [{ch_index}]: {e}")
    return results
...
def _to_peliculas_json_schema(items: list) -> list:
    ...
    for it in (items or []):
        ...
        # ✅ imagen_url: NUNCA debe quedar vacío si hay URL de thumb
        img_final = imagen_url.strip() if isinstance(imagen_url, str) else ""
        if not img_final or img_final == "n/a":
            img_final = "n/a"

        obj = {
            "titulo":            titulo if titulo else "Película",
...
# ---------------------------------------------------------------------------
# GEMINI AI: completa metadatos faltantes + cache IA
# ---------------------------------------------------------------------------
...
async def _gemini_complete_metadata(http, title: str, year, existing_meta: dict):
    ...
    try:
        ...
        if r.status_code == 429:
            ...
            print(f"⚠️  Gemini rate limit (429) para '{title}'")
            return None
...
# ---------------------------------------------------------------------------
# ✅ ENRIQUECIMIENTO PRINCIPAL CON FALLBACK GARANTIZADO A THUMBNAIL NATIVO
# ---------------------------------------------------------------------------
    ...
    async with httpx.AsyncClient(timeout=_timeout) as http:
            ...
            try:
                ...
                if not meta:
                    ...
                    if (not meta) or need_repair:
                        ...
                        # ✅ SIEMPRE generar thumb_img como fallback garantizado
                        ...
                        async with semaphore:
                            tmdb = await (
                                _tmdb_search_and_details(http, query_title, year)
                                if TMDB_API_KEY else _noop()
                            )
                        ...
                        if use_gemini and GEMINI_API_KEY and not (meta.get("sinopsis") and meta.get("generos")):
                            try:
                                ...
                                print(f"   ⚠️  Gemini falló para '{fallback_title}': {_ge}")

                        # ✅ LOG con estado real de imagen
                        has_api_img = bool(meta.get("imagen_url") and not _is_placeholder_image(meta.get("imagen_url")))
                        print(
                            f"   ✅ Enriquecimiento → '{meta.get('titulo', '?')}' "
                            f"img={'✓ (API)' if has_api_img else '✗ (usará /thumb)'} "
                ...
                # ✅ IMAGEN CON FALLBACK GARANTIZADO (orden de prioridad estricto)
                meta_img = meta.get("imagen_url") if isinstance(meta, dict) else None
                if _is_placeholder_image(meta_img):
                    meta_img = None

                # Siempre generar thumb_img (garantizado si hay message ID)
                thumb_img = _thumb_url_for_message(r.get("id"), pelicula_url)
                yt_img    = _youtube_thumb_from_stream_url(pelicula_url)

                # Prioridad: API externa > thumbnail Telegram > YouTube thumb
                ...
                return {
                    ...
                    "imagen_url":            imagen_url,   # ✅ nunca vacío si hay message ID
                    "pelicula_url":          _nn_str(pelicula_url, "n/a"),
                ...
                # ✅ ROBUSTEZ: Nunca romper el JSON por un error en enrich_one
                print(f"⚠️  Error en enrich_one: {_oe}")
                pelicula_url = r.get("stream_url") or ""
                thumb_img    = _thumb_url_for_message(r.get("id"), pelicula_url)
                title_raw    = r.get("title") or "Película"
                clean_t, yr  = _advanced_clean_title(title_raw)
                return {
                    "titulo":                clean_t or "Película",
                    "imagen_url":            thumb_img or "",
                    "pelicula_url":          pelicula_url or "n/a",
                    "descripcion":           "n/a",
                    "fecha_lanzamiento":     "n/a",
                    "duracion":              "n/a",
                    "idioma_original":       "n/a",
        ...
        for item in enriched:
            if isinstance(item, dict):
                ...
                # ✅ Nunca perder una película por excepción inesperada
                final.append({
                    "titulo":                "Película",
                    "imagen_url":            "",
                    "pelicula_url":          "n/a",
...
def _format_results_without_apis(final_results: list, catalog_mode: bool = False) -> list:
    ...
    for r in final_results:
        try:
            ...
            print(f"⚠️  Error en _format_results_without_apis: {_e}")
    return formatted


# ---------------------------------------------------------------------------
# ✅ ENDPOINT /search CON BÚSQUEDA COMBINADA
# ---------------------------------------------------------------------------
@app.get("/search")
async def search(
    ...
    effective_text  = parsed_q.get('text') or (
        q if not parsed_q.get('genre') and not parsed_q.get('year') and not parsed_q.get('desde') else None
    )
    ...
    try:
        ...
        for result in all_results:
            key = normalize_title(result.get("title", ""))
            if key not in seen:
                seen.add(key)
        ...
        if effective_genre and len(final_results) < MIN_CATEGORY_RESULTS:
            ...
            seen_keys = {normalize_title(r.get("title", "")) for r in final_results}
            ...
            if yt_results:
                ...
                if any([effective_year, effective_genre, language, effective_desde, effective_hasta]):
                    enriched = _apply_advanced_filters(
                        enriched, effective_year, effective_genre, language, effective_desde, effective_hasta
                    )
...
async def catalog():
    try:
        ...
        # ✅ FILTRADO: Solo películas que tengan imagen real (incluyendo /thumb/)
        enriched_with_poster = [
            item for item in enriched
            if (
                item.get("imagen_url", "") and
                item.get("imagen_url", "") not in ("n/a", "") and
                not _is_placeholder_image(item.get("imagen_url", ""))
            )
        ]

        # ✅ Si después del filtrado quedaron muy pocas, incluir las que tengan /thumb/
        if len(enriched_with_poster) < 10:
            for item in enriched:
                img = item.get("imagen_url", "")
                if img and img not in ("n/a", "") and item not in enriched_with_poster:
                    enriched_with_poster.append(item)
...
async def youtube_thumbnail_proxy(video_id: str):
    if not video_id or not re.match(r'^[a-zA-Z0-9_\-]{5,20}$', video_id):
    ...
    try:
        ...
        print(f"⚠️  Error procesando ytthumb {video_id}: {e}")
        return Response(content=thumb_data, media_type="image/jpeg")


# ---------------------------------------------------------------------------
# ✅ ENDPOINT /thumb/{message_id} — COMPLETAMENTE REESCRITO Y ROBUSTO
# ---------------------------------------------------------------------------
...
async def get_thumbnail(message_id: int, request: Request, ch: int = 0):
    ...
    Orden de prioridad:
      0. Poster en mensajes adyacentes
      1. message.photo
      2. message.video.thumbs
      3. message.document.thumbs
      4. message.media.thumbs
      5. Placeholder (nunca 404)
    """
    ...
    try:
        ...
        print(f"⚠️  Error leyendo thumb cache [{cache_key}]: {_ce}")

    # ── Obtener cliente activo ─────────────────────────────────────────────
    try:
        active_client = await asyncio.wait_for(get_active_client(), timeout=5.0)
    except asyncio.TimeoutError:
        active_client = None
    except Exception as _ae:
        print(f"⚠️  Error obteniendo cliente activo: {_ae}")
        active_client = None

    if not active_client:
        # Sin cliente, redirigir a placeholder sin levantar excepción
        return Response(status_code=302, headers={"Location": PLACEHOLDER_IMAGE_BASE})
    ...
    try:
        ...
        entity   = (
            ...
            else (app.state.entity if getattr(app.state, "entity", None) is not None else None)
        )
        if entity is None and entities:
            entity = next((e for e in entities if e is not None), None)
    except Exception as _ee:
        print(f"⚠️  Error obteniendo entity para thumb: {_ee}")
        entity = getattr(app.state, "entity", None)
    ...
    try:
        ...
        print(f"   ⚠️  Error obteniendo mensaje {message_id}: {_me}")

    if not message:
        # Mensaje no encontrado → placeholder (no 404)
        return await _serve_placeholder_thumb(thumb_cache, cache_key)

    # ── Descargar miniatura con función robusta ────────────────────────────
    thumb_data = None
    try:
        client_sem = getattr(app.state, "client_semaphore", asyncio.Semaphore(CLIENT_SEM_LIMIT))
        async with client_sem:
            thumb_data = await asyncio.wait_for(
                _download_telegram_thumb_robust(message, entity, active_client, message_id),
                timeout=10.0,
            )
    except asyncio.TimeoutError:
        print(f"   ⚠️  Timeout descargando thumb msg {message_id}")
    except Exception as _te:
        print(f"   ⚠️  Error descargando thumb msg {message_id}: {_te}")

    # ── Procesar y cachear si se obtuvo ───────────────────────────────────
    if thumb_data and len(thumb_data) > 200:
        try:
            ...
            print(f"   ⚠️  Error procesando thumb msg {message_id}: {_pe}")
            return Response(content=thumb_data, media_type="image/jpeg")

    # ── Fallback: placeholder (nunca Error 404 o 500) ─────────────────────
    return await _serve_placeholder_thumb(thumb_cache, cache_key)


async def _serve_placeholder_thumb(thumb_cache: dict, cache_key: str) -> Response:
    """Descarga y devuelve el placeholder, cacheándolo para futuras peticiones."""
    try:
        ph_key = "__placeholder__"
        try:
            async with app.state.thumb_cache_lock:
                ph_cached = thumb_cache.get(ph_key)
                if ph_cached:
                    ts, data, mime = ph_cached
                    if time.monotonic() - ts < THUMB_CACHE_TTL * 10:
                        return Response(content=data, media_type=mime)
        except Exception:
            pass

        async with httpx.AsyncClient(timeout=5.0) as _ph:
            ...
            if _r.status_code == 200 and _r.content:
                ...
                try:
                    async with app.state.thumb_cache_lock:
                        _thumb_cache_prune(thumb_cache)
                        thumb_cache[ph_key]   = (time.monotonic(), processed, mime)
                        thumb_cache[cache_key] = (time.monotonic(), processed, mime)
                except Exception:
                    pass
                return Response(content=processed, media_type=mime)
    except Exception as _phe:
        print(f"   ⚠️  Error descargando placeholder: {_phe}")

    # Último recurso: redirigir a la URL del placeholder
    return Response(status_code=302, headers={"Location": PLACEHOLDER_IMAGE_BASE})
