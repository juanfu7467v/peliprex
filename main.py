...
# ---------------------------------------------------------------------------
# 🔧 FIX PRINCIPAL: _extract_video_frame — SEEK OPTIMIZADO + TIMEOUT 12s
# ---------------------------------------------------------------------------
async def _extract_video_frame(message_id: int, ch: int) -> bytes | None:
    ...
    try:
        ...
        def _run_ffmpeg_via_stream() -> bool:
            # ✅ Reducido a 3 estrategias con timeout máx 12s para no bloquear CPU
            strategies = [
                (10, True,  12),
                (30, True,  12),
                ( 0, True,  10),
...
async def _background_frame_extract(
    ...
    try:
        ...
        # ✅ FALLBACK: FFmpeg solo si no hay miniatura nativa de Telegram
        if not thumb_data:
            is_video = (
            ...
            if is_video:
                print(f"   🔧 Sin miniatura nativa, usando FFmpeg para msg {message_id}...")
