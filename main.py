...
# ---------------------------------------------------------------------------
# 🔧 FIX PRINCIPAL: _extract_video_frame — SEEK OPTIMIZADO (10s/5s) + TIMEOUT 15s
# ---------------------------------------------------------------------------
async def _extract_video_frame(message_id: int, ch: int) -> bytes | None:
    ...
    try:
        ...
        # Si FFmpeg falla, asegurarse de eliminar cualquier archivo residual
...
async def catalog():
    try:
        ...
        # ✅ FILTRADO: Solo guardamos películas que tengan imagen real (poster o miniatura)
