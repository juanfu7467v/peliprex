...
ARCHIVE_STREAM_MAX_CONCURRENCY = max(
    4,
    min(64, int(os.getenv("ARCHIVE_STREAM_MAX_CONCURRENCY", "12")))
)
ARCHIVE_STREAM_CONNECT_TIMEOUT_S = max(
    2.0,
    float(os.getenv("ARCHIVE_STREAM_CONNECT_TIMEOUT_S", "8.0"))
)
ARCHIVE_STREAM_WRITE_TIMEOUT_S = max(
    2.0,
    float(os.getenv("ARCHIVE_STREAM_WRITE_TIMEOUT_S", "15.0"))
)
ARCHIVE_STREAM_POOL_TIMEOUT_S = max(
    0.5,
    float(os.getenv("ARCHIVE_STREAM_POOL_TIMEOUT_S", "5.0"))
)
...
async def lifespan(app: FastAPI):
    ...
    app.state.archive_stream_sem = asyncio.Semaphore(ARCHIVE_STREAM_MAX_CONCURRENCY)
    ...
    print(f"🎬 Archive stream semáforo: max={ARCHIVE_STREAM_MAX_CONCURRENCY}")
...
def _archive_is_private_file(file_item: dict) -> bool:
    if not isinstance(file_item, dict):
        return True
    private_value = str(file_item.get("private") or "").strip().lower()
    return private_value not in ("", "0", "false", "none", "null", "no")



def _archive_build_request_headers(identifier: str, range_header: str | None = None) -> dict:
    ident_q = quote((identifier or "").strip(), safe="")
    headers = {
        "User-Agent": (
        ...
        "Referer": f"https://archive.org/details/{ident_q}" if ident_q else "https://archive.org/",
...
async def _archive_probe_file_access(http, identifier: str, file_name: str) -> bool:
    upstream_url = _archive_build_download_url(identifier, file_name)
    if not upstream_url:
        return False
    ...
    try:
        req = http.build_request(
            "GET",
            upstream_url,
            headers=_archive_build_request_headers(identifier, "bytes=0-1"),
        )
...
def _archive_pick_best_video_file(files: list) -> dict | None:
    ...
    for file_item in normalized_files:
        ...
        if ext in bad_exts:
            continue
        if _archive_is_private_file(file_item):
            continue
...
async def _archive_build_result_from_doc(http, doc: dict, query_title: str, year):
    try:
        ...
        if not archive_download_url or not stream_url:
            return None
        if not await _archive_probe_file_access(http, identifier, file_name):
            return None
...
async def search(
    ...
    if tmdb_gate_required and effective_text:
        ...
        if isinstance(cached_gate_meta, dict) and cached_gate_meta.get("tmdb_id"):
            ...
            async with httpx.AsyncClient(timeout=gate_timeout) as gate_http:
                ...
                elif tmdb_status == "none":
                    print(
                        f"⛔ TMDb no encontró '{gate_query_title}'. "
                        "Se continúa con Archive.org y, si no aparece allí, se busca en Telegram."
                    )
                else:
                    print(
    ...
    try:
        ...
        if effective_text:
            allow_external_fallback = tmdb_gate_status != "none"
            ...
            if allow_external_fallback:
                ...
                print("🟦 TMDb sin coincidencia. Intentando Archive.org antes de Telegram...")
                archive_out = await archive_org_fallback(effective_text.strip())
                external_out = []
            ...
            if preferred_results:
                ...
                if allow_external_fallback:
                    ...
                    print("🟦 Sin resultados en Archive.org. Continuando con Telegram...")
...
async def archive_stream_proxy(identifier: str, request: Request, file: str = Query(..., description="Archivo de Archive.org")):
    ...
    timeout = httpx.Timeout(
        connect=ARCHIVE_STREAM_CONNECT_TIMEOUT_S,
        read=None,
        write=ARCHIVE_STREAM_WRITE_TIMEOUT_S,
        pool=ARCHIVE_STREAM_POOL_TIMEOUT_S,
    )
    limits = httpx.Limits(
        max_connections=1,
    ...
    range_header = request.headers.get("range")
    request_headers = _archive_build_request_headers(ident, range_header)
    ...
    try:
        async with httpx.AsyncClient(
            ...
            probe_request = probe_client.build_request("GET", upstream_url, headers=request_headers)
            probe_response = await probe_client.send(probe_request, stream=True)

            if probe_response.status_code in (401, 403):
                raise HTTPException(status_code=404, detail="Archivo de Archive.org bloqueado o no público")
            if probe_response.status_code in (404, 410):
                raise HTTPException(status_code=404, detail="Archivo de Archive.org no encontrado")
            if probe_response.status_code == 416:
                content_range = probe_response.headers.get("content-range") or "bytes */*"
                return Response(status_code=416, content=b"", headers={"Content-Range": content_range})
            probe_response.raise_for_status()

            content_type = (probe_response.headers.get("content-type") or "video/mp4").split(";", 1)[0].strip() or "video/mp4"
            content_length = probe_response.headers.get("content-length")
            content_range = probe_response.headers.get("content-range")
            etag = probe_response.headers.get("etag")
            last_modified = probe_response.headers.get("last-modified")
        ...
        async def _iterator():
            archive_sem = getattr(app.state, "archive_stream_sem", None)
            upstream = None
            ...
            async def _stream_once():
                ...
                async with httpx.AsyncClient(
                    ...
                    upstream_request = client.build_request("GET", upstream_url, headers=request_headers)
            ...
            try:
                ...
                print(f"⚠️  Stream interrumpido en /archive-stream/{identifier}: {stream_error}")
            finally:
