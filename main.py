...
# ---------------------------------------------------------------------------
# 🚫 FFmpeg ELIMINADO — Miniaturas 100% desde Telegram + APIs externas
# ---------------------------------------------------------------------------
# THUMB_FFMPEG_SEM_LIMIT y FFMPEG_TIMEOUT eliminados (FFmpeg ya no se usa)
...
async def _find_poster_in_nearby_messages(
    ...
    Revisa ±search_range mensajes alrededor del mensaje dado (por defecto ±5).
...
# ---------------------------------------------------------------------------
# 🚫 _extract_video_frame ELIMINADO — FFmpeg removido completamente
#    Las miniaturas se obtienen 100% desde Telegram (nativas) + placeholder
# ---------------------------------------------------------------------------


# ---------------------------------------------------------------------------
# ✅ _background_frame_extract — OBTENCIÓN DE MINIATURA EN SEGUNDO PLANO
#    Estrategia sin FFmpeg:
#    0. Póster en mensajes cercanos (±5 mensajes)
#    1. message.photo (foto del mensaje)
#    2. video.thumbs / document.thumbs / media.thumbs (nativas de Telegram)
#    3. Placeholder (último recurso — siempre devuelve imagen)
# ---------------------------------------------------------------------------
async def _background_frame_extract(
    ...
    try:
        ...
        if not thumb_data:
            try:
                poster = await _find_poster_in_nearby_messages(entity, message_id, client)
        ...
        if thumb_data:
            ...
            # ✅ MEJORA: Usar imagen de respaldo mínima para evitar vacíos en la interfaz
...
async def lifespan(app: FastAPI):
    ...
    # 🚫 app.state.ffmpeg_sem eliminado (FFmpeg ya no se usa)
    ...
    print("🚫 FFmpeg desactivado — miniaturas 100% desde Telegram + placeholder")
...
# ---------------------------------------------------------------------------
# ✅ MULTI-CUENTA: Inicialización de clientes Telegram con rotación Round Robin
# ---------------------------------------------------------------------------
...
def _get_next_client() -> TelegramClient:
    """
    Retorna el siguiente cliente de Telegram en la rotación Round Robin.
    Distribuye las peticiones entre todas las cuentas disponibles.
    Si solo hay una cuenta, siempre retorna esa misma.
    """
...
async def get_thumbnail(message_id: int, request: Request, ch: int = 0):
    try:
        ...
        # 🚫 FFmpeg eliminado — si no hay miniatura nativa, usar placeholder
        print(f"   ℹ️  Sin miniatura nativa para msg {message_id} — usando placeholder")
        try:
            async with httpx.AsyncClient(timeout=4.0) as _ph:
                _r = await _ph.get(PLACEHOLDER_IMAGE_BASE)
                if _r.status_code == 200 and _r.content:
