...
# Máximo de canales a consultar por búsqueda (sin género específico). Evita flood.
...
GENRE_CHANNEL_MAP: dict = {
    ...
    "accion":          [-1003093196542, -1002080388176, -1001506938988],
    "acción":          [-1003093196542, -1002080388176, -1001506938988],
    "aventura":        [-1003093196542, -1002080388176, -1001506938988],
    ...
    "ciencia ficcion": [-1003093196542, -1002080388176],
    "ciencia ficción": [-1003093196542, -1002080388176],
    "sci-fi":          [-1003093196542, -1002080388176],
    "scifi":           [-1003093196542, -1002080388176],
    "ficcion":         [-1003093196542, -1002080388176],
    "ficción":         [-1003093196542, -1002080388176],
    "suspenso":        [-1002080388176, -1003093196542],
    "thriller":        [-1002080388176, -1003093196542],
    ...
    "fantasia":        [-1002080388176, -1003093196542],
    "fantasía":        [-1002080388176, -1003093196542],
    "fantastica":      [-1002080388176, -1003093196542],
    "fantástica":      [-1002080388176, -1003093196542],
    "policial":        [-1002080388176, -1003093196542],
    "crimen":          [-1002080388176, -1003093196542],
    "criminal":        [-1002080388176, -1003093196542],
    "guerra":          [-1002080388176, -1003093196542],
    ...
    "general":         [-1002080388176, -1001506938988, -1003093196542],
}
        ...
        async def _load_one(ch: str):
            async with sem:
                try:
                    ...
                    print(f"✅ Canal cargado: {entity.title}")
                    return entity
                except Exception as ex:
    ...
    try:
        ...
        async def search_in_channel(ch_index: int, entity) -> list:
            ...
            try:
                ...
                _active_client = _get_next_client()
                # ✅ ANTI-FLOOD: Pausa aumentada entre consultas a canales
                await asyncio.sleep(SEARCH_INTER_CHANNEL_DELAY)
                # ✅ Forzar resolución del canal para evitar "Invalid channel object"
                try:
                    _channel_ref = getattr(entity, "id", None) or entity
        ...
        # ✅ ANTI-FLOOD: Limitar canales cuando no hay género ni canal específico
        if not canal and not effective_genre:
            if len(entities_indexed) > SEARCH_MAX_CHANNELS:
                print(
                    f"🔒 Anti-flood: limitando búsqueda a {SEARCH_MAX_CHANNELS} canales "
                    f"(de {len(entities_indexed)} disponibles)"
                )
                entities_indexed = entities_indexed[:SEARCH_MAX_CHANNELS]

        # ✅ ANTI-FLOOD: Búsqueda SECUENCIAL con delay entre canales.
        # Antes: asyncio.gather ejecutaba todo en paralelo → HTTP 429 FloodWait.
        # Ahora: canales consultados uno a uno con pausa entre cada uno.
        all_results: list = []
        for ch_index, entity in entities_indexed:
            channel_results = await search_in_channel(ch_index, entity)
        ...
        # ✅ ANTI-FLOOD: Guardar resultados en caché SQLite para futuras búsquedas idénticas.
        # La próxima vez que se busque lo mismo, no se consultará Telegram.
        if final_schema:
