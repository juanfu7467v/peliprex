...
# ---------------------------------------------------------------------------
# STREAMING (ESTABLE — ANTI-STUTTERING)
# ---------------------------------------------------------------------------
# STREAM_CHUNK_SIZE    : tamaño de cada chunk enviado al cliente (1 MB).
# STREAM_REQUEST_SIZE  : tamaño de cada petición interna de Telethon a Telegram.
#                        2 MB reduce a la mitad los round-trips a Telegram.
# STREAM_MAX_RETRIES   : reintentos ante errores transitorios mid-stream.
# STREAM_PREFETCH_CHUNKS: chunks en cola de pre-carga (buffer adelantado).
# STREAM_MSG_CACHE_TTL : segundos que se cachea el objeto mensaje de Telegram.
#                        Evita llamar get_messages() en cada petición de rango.
...
STREAM_MAX_RETRIES      = max(1, int(os.getenv("STREAM_MAX_RETRIES",      "3")))
STREAM_PREFETCH_CHUNKS  = max(1, int(os.getenv("STREAM_PREFETCH_CHUNKS",  "4")))
STREAM_MSG_CACHE_TTL    = max(60, int(os.getenv("STREAM_MSG_CACHE_TTL",   str(3600))))
...
# ---------------------------------------------------------------------------
# 🚀 CACHÉ DE MENSAJES TELEGRAM (anti-latencia en range requests)
# ---------------------------------------------------------------------------
# Un reproductor de video puede hacer 50-200 peticiones de rango durante la
# reproducción de una película. Sin caché, cada petición llama a
# get_messages() → 1 llamada extra a la API de Telegram por segmento.
# Con caché, get_messages() se llama UNA SOLA VEZ por película por hora.
# ---------------------------------------------------------------------------
_stream_msg_cache: dict = {}   # { "entity_id:msg_id" : (timestamp, message) }
_stream_entity_cache: dict = {}  # { "channel_id" : (timestamp, entity) }


async def _get_stream_message(tg_client, entity, message_id: int):
    """
    Obtiene el mensaje de Telegram con caché en RAM.
    Evita llamadas repetidas a get_messages() durante el streaming de un video.
    """
    eid      = getattr(entity, "id", None) or str(entity)
    key      = f"{eid}:{message_id}"
    now      = time.time()
    cached   = _stream_msg_cache.get(key)

    if cached:
        ts, msg = cached
        if now - ts < STREAM_MSG_CACHE_TTL and msg and getattr(msg, "file", None):
            return msg

    message = await tg_client.get_messages(entity, ids=message_id)
    if message and getattr(message, "file", None):
        _stream_msg_cache[key] = (now, message)
        # Limitar tamaño del caché en RAM
        if len(_stream_msg_cache) > 500:
            oldest = sorted(_stream_msg_cache.items(), key=lambda x: x[1][0])
            for k, _ in oldest[:100]:
                _stream_msg_cache.pop(k, None)
    return message


async def _get_stream_entity(tg_client, raw_entity, ch: int):
    """
    Resuelve y cachea la entidad de canal para streaming.
    Evita llamar get_entity() en cada petición de rango.
    """
    target   = getattr(raw_entity, "id", None) or raw_entity
    key      = str(target)
    now      = time.time()
    cached   = _stream_entity_cache.get(key)

    if cached:
        ts, ent = cached
        if now - ts < STREAM_MSG_CACHE_TTL:
            return ent

    try:
        entity = await tg_client.get_entity(target)
        _stream_entity_cache[key] = (now, entity)
        return entity
    except Exception as _re:
        print(f"⚠️  No se pudo re-resolver entidad ch={ch}: {_re}")
        return raw_entity   # fallback al objeto original


# ---------------------------------------------------------------------------
# 🚀 DESCARGA ESTABLE: reintentos + alineación + trimming exacto
# ---------------------------------------------------------------------------
async def _iter_download_stable(
    tg_client,
    media,
    start:        int,
    length:       int,
    ...
    """
    Generador de descarga estable con:
    • Alineación automática del offset al bloque de request_size.
    • Trimming exacto: devuelve exactamente `length` bytes desde `start`.
    • Reintentos con backoff exponencial; en cada reintento reanuda
      desde el último byte enviado (no desde el principio).
    """
    bytes_sent = 0
    attempt    = 0
    ...
    while bytes_sent < length:
        ...
        abs_offset   = start + bytes_sent
        aligned      = (abs_offset // request_size) * request_size
        pre_skip     = abs_offset - aligned
        dl_limit     = pre_skip + (length - bytes_sent)

        try:
            local_skip = pre_skip
            async for raw_chunk in tg_client.iter_download(
                media,
                offset=aligned,
                limit=dl_limit,
                ...
                # ── Descartar bytes de alineación (solo al inicio del bloque) ──
                if local_skip > 0:
                    if len(raw_chunk) <= local_skip:
                        local_skip -= len(raw_chunk)
                        continue
                    raw_chunk  = raw_chunk[local_skip:]
                    local_skip = 0

                # ── Recortar exactamente al límite del rango ─────────────────
            ...
            return   # descarga completa sin error
            ...
            if attempt > STREAM_MAX_RETRIES:
                print(
                    f"⚠️  Stream abortado tras {STREAM_MAX_RETRIES} reintentos "
                    f"(offset={abs_offset}, enviados={bytes_sent}): {exc}"
                )
                return
            wait_s = min(0.3 * (2 ** attempt), 4.0)
            print(
                f"🔄 Reintento {attempt}/{STREAM_MAX_RETRIES} stream "
                f"(pos={abs_offset}) en {wait_s:.1f}s: {exc}"
            )
            await asyncio.sleep(wait_s)
            # El while vuelve a calcular abs_offset desde bytes_sent actual


async def _prefetch_stream(tg_client, media, start: int, length: int):
    """
    Envuelve _iter_download_stable con una cola de pre-carga (asyncio.Queue).

    El productor descarga de Telegram; el consumidor (FastAPI) envía al cliente.
    La cola absorbe la variación de velocidad de Telegram y elimina micro-pausas.
    Chunks de tamaño fijo (STREAM_CHUNK_SIZE) para buffer estable del reproductor.
    """
    queue: asyncio.Queue = asyncio.Queue(maxsize=STREAM_PREFETCH_CHUNKS + 1)

    async def _producer():
        try:
            buf = bytearray()
            async for data in _iter_download_stable(
                tg_client, media, start, length,
                STREAM_CHUNK_SIZE, STREAM_REQUEST_SIZE,
            ):
                buf.extend(data)
                while len(buf) >= STREAM_CHUNK_SIZE:
                    await queue.put(bytes(buf[:STREAM_CHUNK_SIZE]))
            ...
            print(f"⚠️  Prefetch producer error: {_pe}")
        finally:
            try:
                await queue.put(None)   # señal de fin al consumidor
            except Exception:
                pass
    ...
    try:
        while True:
            try:
                ...
                print("⚠️  Prefetch timeout — deteniendo stream")
                break
            if item is None:
                break
            yield item
...
async def stream_video(message_id: int, request: Request, ch: int = 0):
    try:
        ...
        # ✅ Resolución de entidad con caché — evita get_entity() en cada range request
        entity = await _get_stream_entity(active_client, raw_entity, ch)

        # ✅ Mensaje con caché — evita get_messages() en cada range request
        #    (un video puede generar 50-200 peticiones de rango durante la reproducción)
        message = await _get_stream_message(active_client, entity, message_id)
        ...
        # ── Sin header Range → entregar archivo completo ────────────────────
        if byte_range is None:
            ...
            return StreamingResponse(
                _prefetch_stream(active_client, message.media, 0, file_size),
                status_code=200,
                headers=headers,
                media_type=content_type,
            )

        # ── Con header Range → respuesta 206 Partial Content ────────────────
        ...
        return StreamingResponse(
            _prefetch_stream(active_client, message.media, start, content_length),
            status_code=206,
            headers=headers,
            media_type=content_type,
        )
