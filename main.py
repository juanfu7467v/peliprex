...
async def get_tg_client_and_channel(ch_idx: int):
    ...
    Usa las entidades ya resueltas en app.state.entities (evita re-lookup).
    ...
    # ✅ FIX CRÍTICO: Usar entidades ya resueltas desde app.state.entities
    #    El índice ch corresponde a la posición en entities:
    #    ch=0 → canal principal (@PEELYE), ch=1+ → BACKUP_CHANNELS
    entities = getattr(app.state, "entities", [])
    if not entities:
        print("⚠️  app.state.entities vacío — canales aún no cargados")
        return None, None

    # Si el índice está fuera de rango, usar el primer canal disponible
    if ch_idx < 0 or ch_idx >= len(entities):
        ch_idx = 0

    entity = entities[ch_idx] if ch_idx < len(entities) else None
    if entity is None:
        # Buscar el primer entity no-None disponible
        entity = next((e for e in entities if e is not None), None)
        if entity is None:
            print(f"⚠️  No hay entidad disponible para canal {ch_idx}")
            return None, None
    ...
    for attempt in range(total):
        ...
        try:
            ...
            print(f"⚠️  Cliente {idx} falló para canal {ch_idx}: {e}")
            continue

    print(f"⚠️  Todos los clientes fallaron para canal {ch_idx}")
    return None, None
...
async def lifespan(app: FastAPI):
    ...
    # ✅ MULTI-CUENTA: Conectar todos los clientes disponibles
    ...
    # ✅ FIX 4: Semáforo global para limitar concurrencia de peticiones Telegram
    app.state.client_semaphore = asyncio.Semaphore(MAX_CONCURRENCY)
...
async def get_active_client() -> TelegramClient | None:
    ...
    Usa client_semaphore para limitar concurrencia (máx MAX_CONCURRENCY=15 simultáneas).
...
_NOISE_PATTERNS_ADVANCED = [
    ...
    # Etiquetas de canal/emoji comunes en Telegram
    r"\bestreno\b",
    r"\bnuevo\b",
    r"\bexclusivo\b",
    r"\bcanal\b",
    ...
    r"\bunirse\b",
    r"\bsuscribirse?\b",
    # Etiquetas de calidad adicionales
    r"\bremux\b",
    r"\bhdr(?:10)?\b",
    r"\bdolby\b",
    r"\batmos\b",
...
async def _fetch_recent_media_from_channel(ch_index: int, entity, limit: int) -> list:
    ...
    # ✅ FIX 4: Semáforo global para limitar concurrencia y evitar FloodWait
    _sem = getattr(app.state, "client_semaphore", asyncio.Semaphore(MAX_CONCURRENCY))
    async with _sem:
        ...
        try:
            ...
            print(f"⚠️  Error iterando mensajes canal [{ch_index}]: {_fe}")
    return results
...
async def enrich_results_with_tmdb(
    ...
    async with httpx.AsyncClient(timeout=_timeout) as http:
        ...
        async def enrich_one(r: dict) -> dict:
            ...
            if not meta:
                ...
                if (not meta) or need_repair:
                    if new_counter["n"] >= limit_new:
                        ...
                        # ✅ FIX: Siempre hay imagen — nunca vacío
                        img_final = thumb or yt or PLACEHOLDER_IMAGE_BASE
                        return {
            ...
            if catalog_mode:
                imagen_url = meta_img or thumb_img or yt_img or PLACEHOLDER_IMAGE_BASE
            else:
                imagen_url = meta_img or thumb_img or yt_img or PLACEHOLDER_IMAGE_BASE

            # ✅ Log cuando se usa miniatura nativa de Telegram como fallback
            if not meta_img and thumb_img:
                print(f"   🖼️  Miniatura Telegram como fallback: '{r.get('title', '?')[:40]}' → {thumb_img}")
...
def _format_results_without_apis(final_results: list, catalog_mode: bool = False) -> list:
    ...
    for r in final_results:
        ...
        thumb_img = _thumb_url_for_message(r.get("id"), pelicula_url)
        yt_img    = _youtube_thumb_from_stream_url(pelicula_url)
        # ✅ FIX: Siempre hay imagen válida — /thumb/ → YouTube → placeholder
        img_final = thumb_img or yt_img or PLACEHOLDER_IMAGE_BASE
...
async def catalog():
    try:
        ...
        # ✅ FIX: Incluir items con /thumb/ URL (miniaturas nativas de Telegram) — son imágenes válidas
        #    Solo descartar los que tienen cadena vacía o "n/a" puro sin alternativa
        enriched_with_poster = [
            item for item in enriched
            if item.get("imagen_url", "") not in ("", "n/a")
        ]
