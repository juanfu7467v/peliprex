...
async def get_thumbnail(message_id: int, request: Request, ch: int = 0):
    try:
        ...
        # ✅ FIX 6: PRIORIDAD 1 — miniatura nativa desde message.document.thumbs
        # Esta es la miniatura incrustada en el propio archivo de video por Telegram.
        ...
        # ✅ FIX 6: PRIORIDAD 2 — miniatura nativa desde message.video.thumbs
        if not thumb_data:
        ...
        # ✅ FIX 6: PRIORIDAD 3 — miniatura desde message.media.thumbs (acceso directo al media)
        if not thumb_data:
        ...
        # ✅ FIX 6: PRIORIDAD 4 — miniatura nativa desde message.photo
        ...
        # ✅ FIX 6: PRIORIDAD 5 — buscar póster en mensajes CERCANOS (última opción).
        # Ahora va al FINAL para no mezclar posters entre películas distintas.
        # Solo se usa si no hay ninguna miniatura nativa disponible.
        if not thumb_data:
            try:
                poster = await _find_poster_in_nearby_messages(entity, message_id, active_client)
                if poster and len(poster) > 500:
                    thumb_data = poster
                    print(f"   🖼️  Póster cercano obtenido para msg {message_id}")
            except Exception:
                thumb_data = None
