...
async def _background_frame_extract(
    ...
    try:
        ...
        # ✅ PRIORIDAD 1: miniatura nativa desde message.photo
        ...
        if not thumb_data and hasattr(message, "video") and message.video:
            video_obj = message.video
            thumbs = getattr(video_obj, "thumbs", None) or []
            if thumbs:
                try:
                    best_thumb = sorted(
                        thumbs,
                    ...
                    thumb_data = await asyncio.wait_for(
                        client.download_media(best_thumb, bytes),
                        timeout=4.0,
                    )
                    if thumb_data:
