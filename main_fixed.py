...
MAX_CONCURRENCY           = max(5,  min(15, int(os.getenv("MAX_CONCURRENCY",           "5"))))
...
AI_CACHE_TTL_429_S   = int(os.getenv("AI_CACHE_TTL_429_S",   str(10 * 3600)))
...
# ---------------------------------------------------------------------------
# ✅ FIX 1: DEDUPLICACIÓN DE BÚSQUEDAS EN VUELO (evita race condition)
# Problema: el mismo query se ejecutaba 4 veces en paralelo porque el cliente
# hacía múltiples requests antes de recibir la primera respuesta.
# Solución: registrar búsquedas "en vuelo" y retornar el mismo resultado.
# ---------------------------------------------------------------------------
_search_inflight: dict = {}   # key -> asyncio.Event
_search_results:  dict = {}   # key -> resultado final
...
# ---------------------------------------------------------------------------
# ✅ FIX 3 (parcial): MAPA DE GÉNEROS AMPLIADO con géneros faltantes
# Problema: "politica", "superheroes", "cine independiente" no estaban en el
# mapa, por lo que se buscaba en todos los canales y el timeout eliminaba
# los resultados antes de que el filtro de género pudiera aplicarse.
# ---------------------------------------------------------------------------
GENRE_CHANNEL_MAP: dict = {
    ...
    "religiosa":          ["@PeliculasCristianasBpB", "@Peliculas_Cristianas_Caprichos", "@peliculascristian"],
    "biblica":            ["@PeliculasCristianasBpB", "@Peliculas_Cristianas_Caprichos", "@Series_Bpb"],
    "bíblica":            ["@PeliculasCristianasBpB", "@Peliculas_Cristianas_Caprichos", "@Series_Bpb"],
    ...
    "comedia romantica":  ["@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "comedia romántica":  ["@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    ...
    "belica":             ["@peliculasdetodogeneroo", "@PEELYE"],
    "bélica":             ["@peliculasdetodogeneroo", "@PEELYE"],
    ...
    # ✅ FIX: géneros que antes no existían y retornaban 0 resultados siempre
    "politica":           ["@archivotvcinepiuraperu", "@peliculasdetodogeneroo", "@PEELYE"],
    "política":           ["@archivotvcinepiuraperu", "@peliculasdetodogeneroo", "@PEELYE"],
    "superheroes":        ["@PEELYE", "@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "superhéroes":        ["@PEELYE", "@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "superheroe":         ["@PEELYE", "@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "superhéroe":         ["@PEELYE", "@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "cine independiente": ["@archivotvcinepiuraperu", "@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "independiente":      ["@archivotvcinepiuraperu", "@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "psicologico":        ["@peliculasdetodogeneroo", "@PEELYE", "@pelisdeterror2"],
    "psicológico":        ["@peliculasdetodogeneroo", "@PEELYE", "@pelisdeterror2"],
    "psicologica":        ["@peliculasdetodogeneroo", "@PEELYE", "@pelisdeterror2"],
    "psicológica":        ["@peliculasdetodogeneroo", "@PEELYE", "@pelisdeterror2"],
    "accion aventura":    ["@PEELYE", "@peliculasdetodogeneroo", "@MundoPelisgratis15"],
    "espionaje":          ["@PEELYE", "@peliculasdetodogeneroo"],
    "catastrofe":         ["@PEELYE", "@peliculasdetodogeneroo"],
    "catástrofe":         ["@PEELYE", "@peliculasdetodogeneroo"],
}
...
def _get_best_native_thumb(thumbs_list):
    if not thumbs_list:
        return None
    try:
        from telethon.tl.types import PhotoStrippedSize, PhotoPathSize
...
async def _find_poster_in_nearby_messages(entity, message_id: int, tg_client, search_range: int = 4) -> bytes | None:
    try:
        nearby_ids = list(range(max(1, message_id - search_range), message_id + search_range + 1))
        ...
        for msg in messages:
            ...
            if msg_id is None:
                continue
            messages_with_id.append((abs(msg_id - message_id), msg))
            ...
            if hasattr(msg, "photo") and msg.photo:
                try:
                    ...
                    if photo_data and len(photo_data) > 500:
                        print(f"   🖼️  Póster encontrado en mensaje cercano #{msg.id}")
                        return photo_data
                except Exception:
                    continue
        return None
...
# ---------------------------------------------------------------------------
# MULTI-CUENTA: pre-carga y resolución de entidades
# ---------------------------------------------------------------------------
async def _preload_entities_for_client2():
    ...
    tasks = [_resolve_one(i, uname) for i, uname in enumerate(all_usernames)]
...
async def lifespan(app: FastAPI):
    ...
    for _cl in _telegram_clients:
        try:
            ...
            print(f"⚠️  Error conectando cliente Telegram: {_e}")
    ...
    async def _load_backup_channels():
        ...
        backup_entities = await asyncio.gather(*[_load_one(ch) for ch in BACKUP_CHANNELS])
...
# ---------------------------------------------------------------------------
# ✅ FIX 2: LIMPIEZA DE EMOJIS UNICODE PARA APIs EXTERNAS
# Problema: Títulos con emojis (🔻, 🇲🇽, 𝐋𝐚𝐭𝐢𝐧𝐨) llegaban a TVMaze/TMDB
# causando errores porque las APIs no reconocen esos caracteres.
# ---------------------------------------------------------------------------
def _strip_emojis(text: str) -> str:
    """
    Elimina emojis, flags unicode, caracteres matemáticos bold/italic
    y cualquier símbolo no-ASCII que no sea letra latina extendida.
    """
    if not text:
        return text
    # Rangos: emojis comunes, flags, símbolos misceláneos, Mathematical Alphanumerics
    emoji_pattern = re.compile(
        "["
        "\U0001F600-\U0001F64F"   # Emoticons
        "\U0001F300-\U0001F5FF"   # Símbolos y pictogramas
        "\U0001F680-\U0001F6FF"   # Transporte y mapas
        "\U0001F1E0-\U0001F1FF"   # Banderas (flags)
        "\U00002702-\U000027B0"   # Dingbats
        "\U000024C2-\U0001F251"   # Símbolos adicionales
        "\U0001F926-\U0001F937"
        "\U00010000-\U0010FFFF"   # Bloques suplementarios (incluye Mathematical)
        "\u2640-\u2642"
        "\u2600-\u2B55"
        "\u200d"
        "\u23cf\u23e9\u231a\ufe0f\u3030"
...
# ✅ FIX 4: PATRONES DE RUIDO AMPLIADOS
# Problema detectado en logs:
#   "Fabicantes de lágrimas en"   ← trailing " en"
#   "Terminator: El Exterminador en" ← trailing " en"
#   "Uncharted: Fuera del mapa en"   ← trailing " en"
#   "versión de Jurassic World: Dominion" ← prefix "versión de"
#   "de El Padrino™"              ← prefix "de "
_NOISE_PATTERNS_ADVANCED = [
    # Resoluciones y calidad
    r"\b4k\b", r"\b2k\b", r"\b8k\b",
    r"\b1080p?\b", r"\b720p?\b", r"\b480p?\b", r"\b360p?\b",
    r"\bfull\s*hd\b", r"\bhd\b", r"\bhdts\b", r"\bhdcam\b", r"\bhdrip\b",
    r"\bwebdl\b", r"\bweb[-\s]?dl\b", r"\bweb[-\s]?rip\b",
    r"\bblu[-\s]?ray\b", r"\bbluray\b", r"\bdvdrip\b", r"\bdvdscr\b",
    r"\bbdrip\b", r"\bcam\b", r"\bts\b", r"\bscreener\b",
    # Idiomas y doblaje
    r"\bdoblaje\s+latino\b", r"\bdoblado\s+al?\s+espa[ñn]ol\b",
    r"\baudio\s+latino\b", r"\baudio\s+espa[ñn]ol\b",
    r"\bsub(?:titulo)?s?\s+espa[ñn]ol\b", r"\bsub(?:titulo)?s?\s+español\b",
    r"\bsubtitulado\b", r"\bsubtiulado\b",
    r"\bcastellano\b", r"\blatino\b", r"\bespa[ñn]ol\b",
    r"\bingles\b", r"\benglish\b", r"\bdual\b", r"\bdoblaje\b",
    # ✅ NUEVO: prefijos problemáticos detectados en logs
    r"^versión\s+de\s+",       # "versión de Jurassic World..."
    r"^version\s+de\s+",       # variante sin tilde
    r"^de\s+",                 # "de El Padrino™"
    # ✅ NUEVO: sufijos problemáticos detectados en logs
    r"\s+en\s*$",              # trailing " en" → "Fabicantes de lágrimas en"
    r"\s+\(en\)\s*$",          # trailing " (en)"
    # Texto extra genérico
    r"\bpel[ií]cula\b", r"\bpeliculas?\b", r"\bcompleta\b", r"\bcompleto\b",
    r"\boficial\b", r"\bonline\b", r"\bgratis\b", r"\bfull\b",
    r"\btrailer\b", r"\bremaster(?:ed|izado)?\b", r"\bextended\b",
    r"\bdirectors?\s*cut\b", r"\bunrated\b", r"\bversion\s+extendida\b",
]
...
def _advanced_clean_title(raw_title: str) -> tuple:
    ...
    # 1. Eliminar decoraciones markdown/links
    ...
    # 1.5 Eliminar menciones y extensiones de video
    t = re.sub(r'@\w+', '', t)
    t = re.sub(r'\.(mp4|mkv|avi|mov|wmv|flv|webm|m4v|ts|3gp|mpeg|mpg)(\b|$)', '', t, flags=re.IGNORECASE)

    # 2. Tomar solo la primera línea
    t = t.split('\n')[0].strip()

    # 3. Extraer año ANTES de limpiar
    detected_year = _extract_year_advanced(t)

    # 4. Eliminar números al inicio
    t = re.sub(r'^\d+[\s\.\)\-\]\|×xX]+', '', t).strip()
    t = re.sub(r'^\d+\s*[×xX]\s*\d+\s*', '', t).strip()

    # 5. Eliminar paréntesis/corchetes con ruido
    def _remove_noise_brackets(s: str) -> str:
        def _replacer(m):
            ...
            if len(inner.split()) <= 3:
                ...
                noise_words = ['hd','4k','2k','8k','latino','castellano','español',
    ...
    # 6. Eliminar año detectado del título
    if detected_year:
        t = re.sub(r'\b' + re.escape(detected_year) + r'\b', ' ', t)

    # 7. Aplicar patrones de ruido avanzados (incluye los nuevos FIX 4)
    for pat in _NOISE_PATTERNS_ADVANCED:
        t = re.sub(pat, ' ', t, flags=re.IGNORECASE)

    # 8. Eliminar separadores residuales
    t = re.sub(r'[|•·_,;!?¡¿"\']+', ' ', t)
    t = re.sub(r'\(\s*\)', ' ', t)
    t = re.sub(r'\[\s*\]', ' ', t)

    # 9. Eliminar números sueltos al inicio o al final
    t = re.sub(r'^\d+\s+', '', t).strip()
    t = re.sub(r'\s+\d+$', '', t).strip()

    # 10. Limpiar guiones y espacios
    t = re.sub(r'\s*-\s*$', '', t).strip()
    t = re.sub(r'^\s*-\s*', '', t).strip()
    t = re.sub(r'\s+', ' ', t).strip()

    # 11. Truncar si es muy largo
    ...
    # 12. Capitalizar si quedó en minúsculas
    if t and t == t.lower():
        t = t.title()
...
# ---------------------------------------------------------------------------
# ORDENAMIENTO POR CAPÍTULOS Y SAGAS
# ---------------------------------------------------------------------------
_SAGA_KEYWORDS = [
...
def _detect_saga_name(title: str) -> str | None:
    ...
    for saga in _SAGA_KEYWORDS:
        if normalize_title(saga) in title_norm:
            return normalize_title(saga)
    return None
...
def _sort_results_by_saga_and_chapter(results: list) -> list:
    ...
    for saga_name, saga_items in sorted(saga_groups.items()):
        sorted_with_saga.extend(sorted(saga_items, key=extract_chapter_number))
...
# ---------------------------------------------------------------------------
_NOISE_PATTERNS = [
    r"\bdoblaje\s+latino\b", r"\bcastellano\b", r"\blatino\b",
    r"\bpel[ií]cula\b", r"\bfull\s*hd\b", r"\b1080p\b", r"\b720p\b",
    r"\bhdts\b", r"\bweb[-\s]?dl\b", r"\bblu[-\s]?ray\b",
    r"\baudio\s+latino\b", r"\bsub\s+espa[ñn]ol\b", r"\bsub\s+español\b",
]
...
def _clean_title_for_api(title: str) -> str:
    """
    ✅ FIX 2: Ahora incluye strip de emojis antes de enviar a APIs externas.
    """
    clean, _ = _advanced_clean_title(title)
    # Eliminar emojis que hayan sobrevivido la limpieza avanzada
...
def _build_tmdb_query_from_title(title: str):
    clean_title_adv, detected_year_adv = _advanced_clean_title(title)
    q = clean_title_adv

    # ✅ FIX 2: strip emojis en la query que va a las APIs
    q = _strip_emojis(q)

    if detected_year_adv:
        q = re.sub(r'\b' + re.escape(detected_year_adv) + r'\b', ' ', q).strip()
    ...
    if len(q.strip()) < 3:
        ...
        return (q or raw), (detected_year_adv or year_fallback)

    q = re.sub(r"\s+", " ", q).strip()
    return (q or clean_title_adv), detected_year_adv
...
def _apply_advanced_filters(
    ...
    desde=None,
    hasta=None,
    skip_genre_filter: bool = False,   # ✅ FIX 3: flag para evitar filtro inútil post-timeout
) -> list:
    if not results:
        return results
    ...
    for r in results:
        ...
        # ✅ FIX 3: omitir filtro de género si el enriquecimiento fue por timeout
        # (en ese caso todos los items tienen generos="n/a" y el filtro eliminaría todo)
        if genre_norm and not skip_genre_filter:
...
# ---------------------------------------------------------------------------
async def _google_kg_search(http, query_title: str, year):
...
# TMDB
# ---------------------------------------------------------------------------
async def _tmdb_search_and_details(http, query_title: str, year):
    if not TMDB_API_KEY:
        return None
    try:
        params = {
        ...
        if year:
            by_year = [x for x in candidates
                       if (x.get("release_date") or x.get("first_air_date") or "").startswith(year)]
            if by_year:
                candidates = by_year
        ...
        print(f"   🎬 TMDb → '{title}' [{media_type}] año={year_out} img={'✓' if image_url else '✗'}")
        return {
            "source":                "tmdb",
            "tmdb_id":               tmdb_id,
            "media_type":            media_type,
...
# TVMaze
# ---------------------------------------------------------------------------
async def _tvmaze_fetch(http, query_title: str, year):
    try:
        # ✅ FIX 2: limpiar emojis antes de enviar a TVMaze
        clean_query = _strip_emojis(query_title).strip()
        if not clean_query:
            clean_query = query_title

        r = await http.get(
            f"{TVMAZE_API_BASE}/search/shows",
            params={"q": quote_plus(clean_query)},
        )
        r.raise_for_status()
        items = r.json() or []
        if not items:
...
async def _gemini_complete_metadata(http, title: str, year, existing_meta: dict):
...
# ---------------------------------------------------------------------------
def _merge_metadata_with_kg(kg, tmdb, tvmaze, fallback_title: str, fallback_year) -> dict:
    ...
    # ✅ FIX 5: Para el año, priorizar TMDB (más confiable) sobre KG y TVMaze
    def pick_year():
        # TMDB primero
        if isinstance(tmdb, dict) and tmdb.get("año"):
            return tmdb.get("año")
        # Luego KG
        if isinstance(kg, dict) and kg.get("año"):
            return kg.get("año")
        # Luego TVMaze
        if isinstance(tvmaze, dict) and tvmaze.get("año"):
            return tvmaze.get("año")
        return None
    ...
    return {
        ...
        "año":                   pick_year()                  or fallback_year,
    ...
    async with httpx.AsyncClient(timeout=_timeout) as http:
            ...
            if not meta:
                ...
                if (not meta) or need_repair:
                    ...
                    if use_gemini and GEMINI_API_KEY and not (meta.get("sinopsis") and meta.get("generos")):
                        gemini_data = await _gemini_complete_metadata(http, fallback_title, year, meta)
        ...
        for item in enriched:
            if isinstance(item, dict):
                ...
                final.append({
                    "titulo":"Película","imagen_url":"n/a","pelicula_url":"n/a",
                    "descripcion":"n/a","fecha_lanzamiento":"n/a","duracion":"n/a",
                    "idioma_original":"n/a","popularidad":0,"puntuacion":0,
                    "generos":"n/a","año":"n/a","id":None,"size":"n/a",
                    "descripcion_detallada":"n/a",
                })
        return final
...
# ---------------------------------------------------------------------------
# ✅ FIX 1 + FIX 3 + FIX 6: ENDPOINT /search CORREGIDO
#
# FIX 1: Deduplicación de búsquedas en vuelo (evita 4 requests idénticos)
# FIX 3: skip_genre_filter=True cuando enriquecimiento hace timeout
# FIX 6: Timeout de enriquecimiento aumentado de 4s a 8s
# ---------------------------------------------------------------------------
@app.get("/search")
async def search(
    ...
    if not has_any:
        raise HTTPException(
            status_code=400,
            detail="Se requiere al menos un parámetro: q, year, genre, language, desde, hasta, canal",
        )
    ...
    if q and parsed_q:
        ...
        if not effective_text and (parsed_q.get('genre') or parsed_q.get('year') or parsed_q.get('desde')):
            effective_text = None
        elif effective_text and len(effective_text.strip()) < 2:
            effective_text = None

    if q and not any([parsed_q.get('genre'), parsed_q.get('year'), parsed_q.get('desde')]):
        effective_text = q.strip()

    # ✅ FIX 1: Construir clave única para deduplicar requests idénticos
    dedup_key = f"{effective_text}|{effective_genre}|{effective_year}|{effective_desde}|{effective_hasta}|{canal}"

    if dedup_key in _search_inflight:
        # Otro request idéntico ya está en proceso — esperar su resultado
        event = _search_inflight[dedup_key]
        print(f"♻️  Búsqueda duplicada detectada para '{effective_text}' — esperando resultado en vuelo")
        try:
            await asyncio.wait_for(event.wait(), timeout=12.0)
        except asyncio.TimeoutError:
            pass
        cached_result = _search_results.get(dedup_key)
        if cached_result is not None:
            return cached_result

    # Registrar esta búsqueda como "en vuelo"
    event = asyncio.Event()
    _search_inflight[dedup_key] = event

    try:
        result = await _do_search(
            effective_text, effective_genre, effective_year,
            effective_desde, effective_hasta, canal, language
        )
        _search_results[dedup_key] = result
        # Limpiar cache de resultados después de 5 segundos
        async def _cleanup():
            await asyncio.sleep(5)
            _search_results.pop(dedup_key, None)
        asyncio.create_task(_cleanup())
        return result
    finally:
        _search_inflight.pop(dedup_key, None)
        event.set()
...
async def _do_search(
    ...
    """
    Lógica real de búsqueda, separada del endpoint para permitir deduplicación.
    """
    ...
    enrichment_timed_out = False   # ✅ FIX 3: flag de timeout
    ...
    try:
            ...
            if yt_results:
                try:
                    enriched = await asyncio.wait_for(
                        ...
                        timeout=8.0,  # ✅ FIX 6: aumentado de 4s a 8s
                    )
                except asyncio.TimeoutError:
                ...
                if any([effective_year, effective_genre, language, effective_desde, effective_hasta]):
                    enriched = _apply_advanced_filters(
                        enriched, effective_year, effective_genre, language,
                        effective_desde, effective_hasta,
                        skip_genre_filter=enrichment_timed_out,  # ✅ FIX 3
                    )
        ...
        try:
            ...
            enrichment_timed_out = True  # ✅ FIX 3: marcar timeout
...
async def catalog():
    try:
        ...
        try:
            enriched = await asyncio.wait_for(
                enrich_results_with_tmdb(sample, max_new=MAX_ENRICH_NEW, catalog_mode=True),
                timeout=8.0,
            )
        except asyncio.TimeoutError:
        ...
        # ✅ FILTRADO: Solo items con imagen real (no "n/a" ni "")
...
async def youtube_thumbnail_proxy(video_id: str):
    ...
    if not thumb_data:
        return Response(status_code=302, headers={"Location": PLACEHOLDER_IMAGE_BASE})
            ...
            async def chunk_generator_full(offset: int, limit: int):
                try:
                    async for chunk in cl.iter_download(
                        message.media, offset=offset, limit=limit, chunk_size=STREAM_CHUNK_SIZE,
                    ):
                        yield chunk
